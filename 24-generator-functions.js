...
// ============= //
// ? 23.8 Итак, теперь мы знаем, что функция-генератор возвращает т.н. "итератор" или "объект генератор". Что либо перебираемо в JS, если имеет "протокол итератора". Объект, возвращаемый функцией-генератором перебираемый, но также, например массив, "Set", "Map", строка или даже коллекция DOM-элементов.
// 23.9 Здесь у нас массив продовольственной корзины. Массив перебираемый по умолчанию, на него мы можем использовать либо цикл "for of", либо spread-оператор в массиве.
const groceries = ['apples', 'pasta', 'coffee'];
...
// 23.10.0 Но, что-то, что не является перебираемым, например объект, не имеет таких опций, т.к. у него нет "Symbol.iterator". Попробуем добавить его сами с помощью функции-генератора.
const person = {
  name: 'Lydia',
  age: 21,
  hobbies: ['coding', 'biking'],
  // [Symbol.iterator]: function* () {},
  // Или можно даже описать это короче:
  *[Symbol.iterator]() {
    // 23.10.1 Теперь можно здесь описать, что мы хотим возвращать, когда используем цикл "for of" или используем "spread-оператор" на объект "person". Например, если мы хотим возвращать каждую пару "ключ-значение", как отдельный объект. Мы переберём все ключи в объекте "person" и соберём объект, в котором будут ключ и его значение.
    for (let key in this) {
      yield { [key]: this[key] };
    }
  },
...
// 23.10.2 И вуаля! У нас теперь есть доступ для этого объекта к "for of" циклу и спред-оператору благодаря функции-генератору, что мы ему прописали.
